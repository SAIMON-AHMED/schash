...
\midrule
sch128 & 254 & 508 & 292 & 413 \\
sch192 & 573 & 1146 & 515 & 694 \\
sch256 & 1020 & 2040 & 1061 & 1303 \\
\bottomrule
\end{tabular}
\vspace{0.5em}
\begin{minipage}{0.95\linewidth}
\small
\textbf{Notes.} Security estimates based on generic sponge bounds and algebraic complexity analysis. Interpolation and Gr\"obner basis attack complexities estimated from system degree and variable count. All parameter sets exceed $2^{128}$ security target with substantial margins.
\end{minipage}
\end{table}
...
The SCH-128 parameter set achieves $2^{254}$ collision resistance and $2^{508}$ preimage resistance through its 4-element capacity over a 127-bit prime field. Algebraic attacks require solving systems with estimated degree exceeding 2000, leading to complexity estimates of $2^{292}$ for interpolation and $2^{413}$ for Gr\"obner basis methods.

Higher parameter sets (SCH-192 and SCH-256) provide even larger security margins, exceeding $2^{500}$ bits of security against all analyzed attack vectors. These conservative margins account for potential future advances in algebraic solving techniques.
...
\begin{minipage}{0.95\linewidth}
\small
\textbf{Notes.} Complexity estimates show rapid growth with round count. Even at 1 round, attacks exceed practical feasibility thresholds. Full 7-round version maintains security margins above $2^{250}$ against algebraic methods.
\end{minipage}
\end{table}
...
\begin{itemize}
  \item \textbf{Single-round resistance:} Even 1-round variants resist interpolation attacks due to rapid degree growth ($d_{\text{tri}} = 3$) in the triangular layer. The resulting degree-3 system over 12 variables yields $\approx 2^{25}$ monomials, making interpolation impractical.
  
  \item \textbf{Super-exponential growth:} Algebraic attack complexity grows super-exponentially with round count. At 2 rounds, degree reaches $\approx 9$, pushing Gr\"obner basis complexity to $2^{1008}$ operations. At 3 rounds, complexity exceeds $2^{1500}$.
  
  \item \textbf{Full-round security margins:} The full 7-round SCH-128 maintains security margins of $2^{164}$ bits above the 128-bit target for interpolation attacks and $2^{285}$ bits for Gr\"obner basis attacks, providing substantial security reserves against cryptanalytic improvements.
  
  \item \textbf{Comparison to generic attacks:} All algebraic attack estimates exceed the $2^{254}$ generic collision bound, confirming that attack complexity is dominated by the capacity-based generic security rather than structural vulnerabilities.
\end{itemize}

These results support the claim that symbolic composition induces sufficient algebraic hardness to resist known attack methodologies, while acknowledging that further specialized cryptanalysis remains an open research direction.
...
Code repository: \url{https://github.com/SAIMON-AHMED/schash}.
...
\begin{center}
\href{https://github.com/SAIMON-AHMED/schash}{\texttt{github.com/SAIMON-AHMED/schash}}
\end{center}
